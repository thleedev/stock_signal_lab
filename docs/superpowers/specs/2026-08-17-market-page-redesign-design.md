...
- 한국은행 ECOS 인증키 발급 대기 — 발급 전까지 KR_3Y 는 카탈로그에서 비활성
- KRX OpenAPI 키 발급 후 VKOSPI 포함 여부 검증 — 포함되면 `KR_VOL_20D` 를 교체, 아니면 유지
- FreeSIS 신용잔고 요청 스펙 캡처 — 실패 시 신용잔고를 이번 범위에서 제외
- `BAMLH0A0HYM2` API 키 경로에서 2015년까지 열리는지 확인
- GOLD·EWY 의 2015년 이후 Yahoo 백필 가능 여부 — Yahoo 429 재현성에 달림
