...
- ~~FreeSIS 신용잔고 요청 스펙 캡처~~ → **확보 (2026-08-20).** Playwright 로 화면 요청을 캡처해 POST `/meta/getMetaDataList.do`(OBJ_NM `STATSCU0100000070BO`) 스펙을 얻었고, 무키·무세션 curl 재현을 확인했습니다. CREDIT_BALANCE 를 200일 이격도 판정으로 활성화하고 2015-01-02~ 전 구간(2,865행)을 백필했습니다.
