...
값이 없는 읽기 전용 항목은 자리를 차지하지 않게 하되, 편집 가능 필드는 값이 없어도
라벨과 편집 진입점을 유지합니다. 구매가·손절가·목표가는 `renderEditablePrice`로 그리는
편집 가능 필드이며 라벨과 연필 아이콘이 값을 입력하는 유일한 경로입니다. 값이 없다고
숨기면 사용자가 값을 새로 입력할 방법이 사라지므로 이 세 필드는 예외로 둡니다.
...
최종 검토에서 두 가지가 새로 드러났습니다. 첫째, `/api/v1/prices` GET 라우트가 없어
실시간 시세 갱신이 사이트 전역에서 동작하지 않습니다. 존재하는 것은 `prices/refresh`
하나뿐이고 리라이트도 없습니다. `usePriceRefresh` 소비처 8곳이 모두 해당합니다.
실패가 `r.ok` 검사로 흡수되어 캐시 값으로 폴백하므로 화면은 깨지지 않지만 실시간
갱신 기능 자체가 죽어 있습니다. 다음 사이클 최우선입니다.

둘째, 모바일 카드에서 가격 편집 진입점(연필 아이콘)이 `opacity-0 group-hover`라 터치
기기에서 보이지 않습니다. `investment-client.tsx:291`과 `my-portfolio/page.tsx:344`
두 곳이며 함께 고쳐야 합니다. 카드로 값을 보이게 만들었는데 편집은 못 하는 상태입니다.
