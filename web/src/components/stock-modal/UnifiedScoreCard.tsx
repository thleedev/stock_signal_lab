...
/** 체크리스트 카테고리 ID별 표시 메타 */
const CHECKLIST_CATEGORY_META: Record<
  AnalysisCategory['id'],
  { color: string; text: string }
> = {
  technical:  { color: 'bg-emerald-500', text: 'text-emerald-500' },
  supply:     { color: 'bg-sky-500',     text: 'text-sky-500'     },
  valuation:  { color: 'bg-violet-500',  text: 'text-violet-500'  },
  signal:     { color: 'bg-amber-500',   text: 'text-amber-500'   },
  risk:       { color: 'bg-red-500',     text: 'text-red-500'     },
};
...
// ── 체크리스트 아이템 목록 (API 응답용) ──────────────────────────────────────
function ChecklistItemList({ reasons }: { reasons: AnalysisCategory['reasons'] }) {
  if (!reasons || reasons.length === 0) {
    return <p className="text-xs text-[var(--muted)] py-1">산출 근거 없음</p>;
  }
  const passed = reasons.filter((r) => r.passed);
      ...
      {passed.map((r, i) => (
        <div key={i} className="flex items-start gap-2 text-xs">
          ...
          <div className="min-w-0">
            <span className="font-medium text-[var(--text)]">{r.label}</span>
            {r.value && (
              <p className="text-[var(--muted)] leading-snug mt-0.5">{r.value}</p>
            )}
          </div>
        </div>
      ))}
...
// ── 체크리스트 카테고리 아코디언 (API 응답용) ────────────────────────────────
function ChecklistCategoryAccordion({ category }: { category: AnalysisCategory }) {
  const [open, setOpen] = useState(false);
  const meta = CHECKLIST_CATEGORY_META[category.id];
  ...
  return (
    <div className="border border-[var(--border)] rounded-lg overflow-hidden">
      ...
      >
        {/* 카테고리명 */}
        <span className="text-sm font-medium w-16 shrink-0">{category.label}</span>
        ...
        {/* 점수 숫자 */}
        <span className={`tabular-nums text-sm font-bold w-8 text-right shrink-0 ${meta.text}`}>
          {category.score}
        </span>

        {/* 충족 조건 수 */}
        <span className="text-xs text-[var(--muted)] w-14 text-right shrink-0">
          {passedCount}/{category.reasons.length}개
        </span>
      ...
      {open && (
        <div className="px-3 pb-3 border-t border-[var(--border)] bg-[var(--background)]">
          <ChecklistItemList reasons={category.reasons} />
          {category.id === 'risk' && category.score > 0 && (
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/stock-analysis?symbol=${encodeURIComponent(data.symbol)}`, {
      ...
      .catch((err) => {
        ...
        console.error('[UnifiedScoreCard] 체크리스트 로드 실패:', err);
        ...
        {!analysisLoading && analysisData && (
          <div className="space-y-1.5">
            {analysisData.categories.map((category) => (
              <ChecklistCategoryAccordion key={category.id} category={category} />
            ))}
          </div>
        )}
      </div>
